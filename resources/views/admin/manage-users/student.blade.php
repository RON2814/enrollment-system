        ...
        <div class="bg-white mt-4 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 p-8">
            <div class="flex items-center justify-between mb-5">
                ...
                <div class="flex space-x-4">
                    ...
                    <!-- Filter Dropdown -->
                    <select id="program"
                        class="px-8 pl-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <tbody class="text-gray-700">
                        @foreach ($students as $student)
                            <tr class="hover:bg-gray-100 transition-colors duration-200">
                                ...
                                <td class="py-4 px-4 text-sm truncate max-w-xs">{{ strtoupper($student->last_name) }},
                                    {{ strtoupper($student->first_name) }} {{ strtoupper($student->middle_name) }}</td>
                                <td class="py-4 px-4 text-sm truncate max-w-xs">{{ $student->contact_number }}</td>
                                <td class="py-4 px-4 text-sm truncate max-w-[200px] break-words">
                                    @if ($student->address)
