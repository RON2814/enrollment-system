        ...
        <div class="bg-white mt-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 p-8 py-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-2 text-sm">
                ...
                <div>
                    ...
                    <p class="mt-1 px-2 py-1 border rounded-lg capitalize bg-gray-200 bg-opacity-65">{Year Level}</p>
                </div>
                <div>
                    <p class="font-medium">Semester:</p>
                    <p class="mt-1 px-2 py-1 border rounded-lg capitalize bg-gray-200 bg-opacity-65">{Semester}</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <p class="mt-1 px-2 py-1 border rounded-lg capitalize bg-gray-200 bg-opacity-65">{{ $student->sex }}
                    </p>
                </div>
                ...
                @if ($has_data)
                    <div
                        ...
                        <div class="overflow-x-auto mt-0 rounded-lg-md border">
                            <form action="{{ route('registrar.checklist', ['student_number' => $student->student_number]) }}" method="POST">
                                @csrf
                                @method('PATCH')
                                        ...
                                        @foreach ($checklist as $item)
                                            @if ($item->year == $year_level && $item->semester == $semester)
                                                <tr class="hover rounded-lg transition-colors duration-200">
                                                    <td class="py-4 px-4 text-sm font-semibold">{{ $item->course_code }}</td>
                                                    <td class="py-4 px-4 text-sm">{{ $item->course->course_title }}</td>
                                                    ...
                                                    <td class="py-4 px-4 text-sm">{{ explode(' ', $item->semester)[0] ?? '' }}</td>
                            
                                                    <!-- Grade Selection -->
                                                    <td class="py-4 px-4 text-sm text-center w-[100px]">
                                                        <input list="gradeOptions" name="grades[{{ $item->course_code }}]" class="border p-1 rounded-lg w-full" value="{{ $item->grade }}">
                                                    ...
                                                    <td class="py-4 px-4 text-sm w-[180px] text-xs">
                                                        <select name="instructors[{{ $item->course_code }}]" class="border p-1 rounded-lg w-full">
                                                            <option value="">Select Instructor</option>
                                                            @foreach ($instructors as $instructor)
                                                                <option value="{{ $instructor->id }}" 
                                                                    {{ $item->instructor_id == $instructor->id ? 'selected' : '' }}>
